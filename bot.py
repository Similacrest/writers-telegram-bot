...
class PromptsBot:
    ...
    def check_if_chat_whitelisted(self, chat):
        return chat.type == "private" or (
            chat.type in ["group", "supergroup"] and chat.id in self.welcome_texts.keys()
        )
    ...
    async def process_message(self, update, context):
        if "слава україні" in utils.text_or_caption(update.effective_message).lower():
            ...
            await update.effective_message.reply_html(
                random.choices(["<i>Смерть ворогам!</i>", "Пизда ᵖосійській ᶲедерації!"], weights=[0.8, 0.2])[0]
            )
...
def main():
    ...
    app.add_handler(MessageHandler(filters.TEXT, bot_logic.process_message))
